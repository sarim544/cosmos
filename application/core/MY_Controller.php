...
class MY_Controller extends CI_Controller {
    ...
    public function isMemLogged($type, $is_verified = true, $sitter_check = true, $type_arr = array('owner', 'sitter'), $memberhsip_check = true) {
        /*if (intval($this->session->mem_id)<1 || !$this->session->has_userdata('mem_type') || $this->session->mem_type!=$type) 
        {
            $remember_cookie=$this->input->cookie('pfsc_remember');
        ...
        if($is_verified)
            $this->is_verified();
        if($sitter_check && $this->session->mem_type=='sitter' && $this->data['mem_data']->mem_sitter_application==0){
            redirect('become-a-sitter', 'refresh');
                exit;
        }
        if($sitter_check && $this->session->mem_type=='owner' && $this->data['mem_data']->mem_become_owner==0){
            redirect('become-pet-owner', 'refresh');
                exit;
        }
        if($this->session->mem_type=='owner' && $this->data['mem_data']->mem_verified==1 && empty($this->data['mem_data']->mem_package_id) && $memberhsip_check){
