...
class Withdrawals extends Admin_Controller {
    ...
    public function index() {
        ...
        $this->data['pageView'] = ADMIN . '/withdrawal-requests';
        $this->data['page_title'] = 'Withdrawals';
    ...
    public function requests() {
        $this->data['enable_datatable'] = TRUE;
        $this->data['pageView'] = ADMIN . '/withdrawal-requests';
        $this->data['page_title'] = 'Withdrawal Requests';
    ...
    function detail($id=0) {
        ...
        if($this->data['row'] = $this->transaction_model->get_withdraw($id)){
            $this->data['with_trxs'] = $this->transaction_model->get_withdrawal_detail($id);
            $this->data['pageView'] = ADMIN.'/withdrawal-requests';
            ...
            $this->data['page_title'] = $this->data['row']->status==1?'Withdrawal':'Withdrawal Request';
            $this->load->view(ADMIN.'/includes/siteMaster', $this->data);
        }
        else
            show_404();
        ...
        if($row = $this->transaction_model->get_withdraw($id, array('status'=>0))){
            $save_data = array('status' => 1, 'paid_date' => date('Y-m-d h:i:s'));
                ...
                if ($bank<1 || !$this->payment_methods_model->get_mem_method($bank, $row->mem_id)) {
                    setMsg('error', 'Bank account not belongs to that sitter!');
