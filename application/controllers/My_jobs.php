        ...
        if($this->input->post()){
            ...
            if (count($jobs) > 0) {
                ...
                foreach ($jobs as $key => $job){
                    ...
                    <div class="ico"><img src="'.get_image_src($job->mem_image,150,true).'"></div>
                    <div class="job-meta">
                    <div class="title">
    ...
    function add_new() {
        ...
        if($this->input->post()){
            ...
            if($this->form_validation->run()===FALSE)
            {
                ...
                $save_data=array('mem_id'=>$this->session->mem_id,'title'=>$post['title'],'subject'=>$post['subject'],'grade_level'=>$post['grade_level'],'budget'=>$post['budget'],'city'=>$post['city'],'state'=>$post['state'],'zip'=>$post['zip'],'detail'=>$post['detail'],'status'=>1,'date'=>date("Y-m-d H:i:s"));
    ...
    function edit(){
        ...
        if($this->job_model->get_row_where(array('id'=>$id,'mem_id'=>$this->session->mem_id)) && $this->input->post()){
            ...
            if($this->form_validation->run()===FALSE)
            {
                ...
                $save_data=array('title'=>$post['title'],'subject'=>$post['subject'],'grade_level'=>$post['grade_level'],'budget'=>$post['budget'],'city'=>$post['city'],'state'=>$post['state'],'zip'=>$post['zip'],'detail'=>$post['detail']);
