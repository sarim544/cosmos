...
class Uploader extends MY_Controller {
	...
	function __construct() {
		parent::__construct();
		// $this->isMemLogged($this->session->mem_type, true, false);
		$this->isMemLogged($this->session->mem_type, true, false, array('buyer', 'player'), false);
		// $this->isMemLogged('buyer', true, false, array('buyer', 'buyer'), false);
	...
	function save_mul_images()
	{
		$res['upload_status'] = 0;
		// $token=doDecode($this->input->post('pk_key'));
		if (/*($this->master->getRow('members',array('mem_token'=>$token)) || $token=='admin') && */count($_FILES['images']['name'])>0 && $_FILES['images']['name'][0]!='' && ($this->master->getRow('characters', array('id', intval($this->input->post('character')))) || in_array($this->input->post('type'), array('character', 'product')))) {
			...
			foreach ($_FILES['images']['name'] as $key => $file_name) {
				...
				if($_FILES['image_file']['name']!=''){
					...
					if (!empty($image['file_name'])) {
						$this->master->save('gallery', array('image' => $image['file_name'], 'ref_type' => $this->input->post('type'), 'ref_id' => $this->input->post('character'), 'mem_id' => $this->session->mem_id, 'date' => date('Y-m-d H:i:s')));
