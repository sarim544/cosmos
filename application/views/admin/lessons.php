            ...
            <?php if (count($rows) > 0): $count = 0; ?>
                <?php foreach ($rows as $row): ?>
                    <tr class="odd gradeX">
                        ...
                        <td><b><a href="<?= site_url(ADMIN.'/students/manage/'.$row->student_id); ?>" target="_blank"><?= get_mem_name($row->student_id); ?></a></b></td>
                        <td><b><a href="<?= site_url(ADMIN.'/tutors/manage/'.$row->tutor_id); ?>" target="_blank"><?= get_mem_name($row->tutor_id); ?></a></b></td>
                        <td><?= format_date($row->lesson_date_time,'M d, Y h:i:m a'); ?></td>
                        <td><?= format_date($row->date,'M d, Y h:i:m a'); ?></td>
                        <td>$ <?= $row->amount; ?></td>
                        <td class="text-center">
                            <a href="<?= site_url(ADMIN.'/lessons/detail/'.$row->id); ?>" class="btn btn-primary btn-sm">Detail</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php endif; ?>
